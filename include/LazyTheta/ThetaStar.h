    ...
    PathDescriptor Search(const JPSL::Point& startPoint, const JPSL::Point& goalPoint, const std::string& expansionPolicy, float TimeLimit, bool debug = false) override
    {
        ...
        while (!openList.empty())
        {
            ...
            if (currentNode->coord == goal)
            {
                ...
                return descriptor; // Assuming expanded nodes are the same as visited nodes
            ...
            for (const auto& neighbor : Neighbours(currentNode->coord, expansionPolicy))
            {
                ...
                if (it == allNodes.end() || parentCoordNode == allNodes.end() || !HasVisibility(parentCoordNode->first, neighborCoord))
                {
                    if (!HasVisibility(currentNode->coord, neighborCoord))
                    {
                        continue;
                    }
                    ...
                    {
                        auto neighborNode = std::make_shared<Node>(neighborCoord, tentativeG, HeuristicFunction(neighborCoord, goal), currentNode->coord);
				...
				else
                {
                    ...
                    float tentativeG = parentCoordNode->second->g + Euclidian(parentCoordNode->first, neighborCoord);

                    if (it == allNodes.end() || tentativeG < it->second->g)
                    {
                        auto neighborNode = std::make_shared<Node>(neighborCoord, tentativeG, HeuristicFunction(neighborCoord, goal), parentCoordNode->first);
                        openList.push(neighborNode);
                        allNodes[neighborCoord] = neighborNode;
                    }
                }
