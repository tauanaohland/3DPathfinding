...
class ThetaStar : public ISearcher
{
...
public:
    // Constructor
    ThetaStar(float (*HeuristicFunction)(const Coord3D& a, const Coord3D& b))
    {
    ...
    std::vector<Coord3D> ReconstructPath(std::shared_ptr<Node> currentNode)
    {
        std::vector<Coord3D> path;
        for (std::shared_ptr<Node> node = currentNode; node != nullptr; node = node->parent.lock())
        {
    ...
    PathDescriptor Search(const JPSL::Point& startPoint, const JPSL::Point& goalPoint, const std::string& expansionPolicy, float TimeLimit, bool debug) override
    {
        ...
        while (!openList.empty())
        {
            ...
            {
                std::vector<Coord3D> path = ReconstructPath(currentNode);
                ...
                allNodes.clear();
                closedList.clear();
                PathDescriptor descriptor(totalCost, visitedNodes + closedList.size(), path.size(), totalTime, pathLength); // Assuming expanded nodes are the same as visited nodes
            ...
            for (const auto& neighbor : Neighbours(currentNode->coord, expansionPolicy))
            {
                ...
                if (it == allNodes.end() || currentNode->parent.lock() == nullptr || !HasVisibility((currentNode->parent.lock())->coord, neighborCoord))
                {
                    // If there is no visibility from the parent node to the neighbor node, use the standard A* expansion.
                    float tentativeG = currentNode->g + neighbor.second;

                    if (closedList.find(neighborCoord) != closedList.end())
                    {
                        continue;
                    }
                    ...
                    {
                        auto neighborNode = std::make_shared<Node>(neighborCoord, tentativeG, HeuristicFunction(neighborCoord, goal), currentNode);
                ...
                {
                    // If there is visibility, perform a lazy evaluation.
                    float tentativeG = (currentNode->parent.lock())->g + Euclidian((currentNode->parent.lock())->coord, neighborCoord);

                    if (it == allNodes.end() || tentativeG < it->second->g)
                    {
                        auto neighborNode = std::make_shared<Node>(neighborCoord, tentativeG, HeuristicFunction(neighborCoord, goal), currentNode->parent.lock());
                        openList.push(neighborNode);
                        allNodes[neighborCoord] = neighborNode;
                    }
                }
